...
Item {
    ...
    PaintTransition {
        id: compressionTransition
        targetValue: root.paint.pressed && !root.paint.surface && !root.paint.field ? root.motion.pressScale : 1
        motion: root.motion
    }
    scale: compressionTransition.value
    ...
    BezelStroke {
        objectName: "liquidBezel"
        anchors.fill: parent; geometry: root.geometry
        strength: root.sheen
    }
    PaintShape {
        objectName: "liquidSpecular"
        anchors.fill: parent; geometry: root.geometry
        gradient: Gradient {
            GradientStop { position: 0; color: Qt.rgba(1,1,1,root.sheen * .32) }
            GradientStop { position: .18 + root.interactionProgress * .12; color: "transparent" }
            GradientStop { position: 1; color: Qt.rgba(1,1,1,root.sheen * .14) }
        }
    }
    PaintShape {
        anchors.fill: parent; geometry: root.geometry
