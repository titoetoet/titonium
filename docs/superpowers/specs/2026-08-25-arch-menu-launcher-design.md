...
The grid is fixed at six columns and four rows. Every page contains at most 24 application cells;
tile dimensions are derived from the available content area without changing the 6×4 structure.
...
- `AppsPage.qml`: fixed 6×4 paging and application presentation.
...
- Only the current section is loaded; Info probes and Power confirmation UI do
...
- Apps are alphabetic and paginated in a fixed 6×4 grid with correct one-line elision and indicators.
...
1. Arch Menu shell, left rail and fixed 6×4 Apps.
2. Extract `SettingsWorkspace` and host it in both Settings Center and Arch Menu.
3. Add Info with visibility-gated Platform data.
4. Design and add Power using the existing session-action boundary.
...
- Implementing a file browser; the installed Yazi remains the file-navigation tool.
